...
from vllm_mblt.mblt_platform import (
    MbltPlatform,
    resolve_effective_npu_prefill_chunk_size,
    resolve_model_max_batch_size,
    resolve_npu_prefill_chunk_size,
)
...
class TestMbltPlatformPrefill:
    ...
    def test_prefers_json_string_loader_core_mode_over_hf_core_mode(self) -> None:
        config = _make_vllm_config(
            ...
            loader_extra_config=json.dumps({"core_mode": "global4"}),
            hf_core_mode="single",
        )
        assert resolve_npu_prefill_chunk_size(config) == 256
    ...
    def test_resolves_json_string_loader_max_batch_size_override(self) -> None:
        config = _make_vllm_config(
            {"global4": 256},
            loader_extra_config=json.dumps({"core_mode": "global4", "max_batch_size": 16}),
            max_batch_size=32,
        )
