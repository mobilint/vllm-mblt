...
from vllm_mblt.mblt_worker import CacheSnapshot, MbltWorker, RequestState, SnapshotIndexNode, _is_qwen3_vl_hf_config, _is_multimodal_hf_config
...
class TestMbltWorkerOptimizations:
    ...
    def test_multimodal_hf_config_ignores_architecture_heuristics(self) -> None:
        assert not _is_multimodal_hf_config(SimpleNamespace(model_type='some_vision_encoder', architectures=['SomeVisionModel']))
        assert not _is_multimodal_hf_config(SimpleNamespace(model_type='custom_text_model', architectures=['FooVLForConditionalGeneration']))
        assert not _is_multimodal_hf_config(SimpleNamespace(model_type='vision_with_text_tower', vision_config=SimpleNamespace()))
    ...
    def test_mobilint_vlm_request_constraints_reject_multiple_images(self) -> None:
        ...
        with pytest.raises(RuntimeError, match='exactly one image'):
            worker._validate_mobilint_vlm_request_constraints([self._make_mm_feature('image', offset=1, length=2), self._make_mm_feature('image', offset=3, length=2)], session_id='session-a')
    ...
    def test_mobilint_vlm_request_constraints_reject_video(self) -> None:
        ...
        with pytest.raises(RuntimeError, match='does not support video'):
            worker._validate_mobilint_vlm_request_constraints([self._make_mm_feature('video', offset=1, length=2)], session_id='session-a')

    def test_mobilint_vlm_request_constraints_reject_image_position_change_in_same_session(self) -> None:
        worker = self._make_worker()
        worker.model_config.hf_config = SimpleNamespace(model_type='mobilint-qwen2_vl')
        worker._validate_mobilint_vlm_request_constraints([self._make_mm_feature('image', offset=1, length=2)], session_id='session-a')
        with pytest.raises(RuntimeError, match='fixed image-token position'):
            worker._validate_mobilint_vlm_request_constraints([self._make_mm_feature('image', offset=2, length=2)], session_id='session-a')

    def test_mobilint_vlm_request_constraints_allow_different_positions_across_sessions(self) -> None:
        worker = self._make_worker()
        worker.model_config.hf_config = SimpleNamespace(model_type='mobilint-qwen2_vl')
        worker._validate_mobilint_vlm_request_constraints([self._make_mm_feature('image', offset=1, length=2)], session_id='session-a')
        worker._validate_mobilint_vlm_request_constraints([self._make_mm_feature('image', offset=4, length=3)], session_id='session-b')
    ...
    def test_mobilint_vlm_request_constraints_include_is_embed_in_fixed_position(self) -> None:
        worker = self._make_worker()
        worker.model_config.hf_config = SimpleNamespace(model_type='mobilint-qwen3_vl')
        worker._validate_mobilint_vlm_request_constraints([self._make_mm_feature('image', offset=1, length=3, is_embed=torch.tensor([True, False, True]))], session_id='session-a')
    ...
    def test_mobilint_vlm_request_constraints_are_noop_for_non_mobilint_models(self) -> None:
        worker = self._make_worker()
        worker.model_config.hf_config = SimpleNamespace(model_type='qwen2_vl')
        worker._validate_mobilint_vlm_request_constraints([self._make_mm_feature('video', offset=1, length=2), self._make_mm_feature('image', offset=3, length=2)], session_id='session-a')
    ...
    def _make_worker(self) -> MbltWorker:
        ...
        worker.vllm_config = SimpleNamespace(cache_config=SimpleNamespace(block_size=128), model_config=worker.model_config)
    ...
    def _make_request_state(self, worker: MbltWorker, sampling_params: SamplingParams, prompt_token_ids: list[int], *, output_token_ids: list[int] | None=None) -> RequestState:
        return RequestState(is_prefill=False, output_token_ids=output_token_ids or [], sampling_params=sampling_params, cached_sampling_state=worker._make_cached_sampling_state(sampling_params, prompt_token_ids), block_ids=([],), first_seq_blocks=(), num_computed_tokens=0, num_output_tokens=0, prompt_embeds=np.empty((0, 1), dtype=np.float32), prompt_deepstack_embeds=None, is_multimodal=False, prompt_len=0, prompt_token_ids=prompt_token_ids, cache_slot_id=None, vlm_session_id=None)

    def _make_mm_feature(self, modality: str='image', *, offset: int=1, length: int=2, is_embed: torch.Tensor | None=None) -> SimpleNamespace:
        return SimpleNamespace(modality=modality, data={}, mm_position=SimpleNamespace(offset=offset, length=length, is_embed=is_embed))
    ...
    def test_snapshot_index_can_prefer_shallower_prefix_with_more_tokens(self) -> None:
        ...
        deep_but_short = CacheSnapshot(blobs=[], block_ids=([1, 2, 3, 7],), first_seq_blocks=(1, 2, 3, 7), num_tokens=100)
    ...
    def test_sampling_penalties_can_be_forced_off_for_non_cuda_runtime(self) -> None:
        ...
        sampling_params = SamplingParams.from_optional(frequency_penalty=0.5, presence_penalty=0.2, repetition_penalty=1.1)
        req_state = self._make_request_state(worker, sampling_params, [1, 2, 3])
        metadata = worker._make_sampling_metadata([req_state])
        assert metadata.no_penalties
        assert metadata.prompt_token_ids is None
    ...
    def test_build_prompt_embeds_keeps_prompt_embeds_without_mm_features(self) -> None:
        ...
        merged, deepstack = worker._build_prompt_embeds(prompt_token_ids=None, prompt_embeds=prompt_embeds, mm_features=None)
    ...
    def test_build_prompt_embeds_scatters_mm_features_into_prompt_embeds(self) -> None:
        ...
        feature = SimpleNamespace(modality='image', data={'pixel_values': torch.zeros(1, 3), 'image_grid_thw': torch.tensor([1, 1, 1])}, mm_position=SimpleNamespace(offset=1, length=2, is_embed=None))
        merged, deepstack = worker._build_prompt_embeds(prompt_token_ids=None, prompt_embeds=base_prompt_embeds, mm_features=[feature])
    ...
    def test_build_prompt_embeds_ignores_deepstack_outputs_for_non_qwen3_vl(self) -> None:
        ...
        worker.model = SimpleNamespace(config=SimpleNamespace(vocab_size=32000, model_type='qwen2_vl'), get_image_features=lambda **_kwargs: ((image_embeds,), deepstack_layers))
        feature = SimpleNamespace(modality='image', data={'pixel_values': torch.zeros(1, 3), 'image_grid_thw': torch.tensor([1, 1, 1])}, mm_position=SimpleNamespace(offset=1, length=2, is_embed=None))
        merged, deepstack = worker._build_prompt_embeds(prompt_token_ids=None, prompt_embeds=base_prompt_embeds, mm_features=[feature])
        assert merged is not base_prompt_embeds
        torch.testing.assert_close(merged[1:3], image_embeds)
    ...
    def test_build_infer_inputs_adds_zero_deepstack_for_qwen3_vl_dual_input_model(self) -> None:
        worker = self._make_worker()
        worker.model_config.hf_config = SimpleNamespace(model_type='mobilint-qwen3_vl')
        worker.cache_model = SimpleNamespace(get_num_model_variants=lambda: 1, get_model_variant_handle=lambda _idx: SimpleNamespace(get_model_input_shape=lambda: [(1, -1, 4), (3, -1, 4)]))
    ...
    def test_build_infer_inputs_rejects_invalid_deepstack_shape(self) -> None:
        worker = self._make_worker()
        worker.model_config.hf_config = SimpleNamespace(model_type='mobilint-qwen3_vl')
        worker.cache_model = SimpleNamespace(get_num_model_variants=lambda: 1, get_model_variant_handle=lambda _idx: SimpleNamespace(get_model_input_shape=lambda: [(1, -1, 4), (3, -1, 8)]))
        input_embeds = np.ones((5, 4), dtype=np.float32)
    ...
    def test_infer_logits_passes_deepstack_to_dual_input_model(self) -> None:
        ...
        worker.cache_model = SimpleNamespace(get_num_model_variants=lambda: 1, get_model_variant_handle=lambda _idx: SimpleNamespace(get_model_input_shape=lambda: [(1, -1, 4), (2, -1, 4)]), infer=infer, get_model_output_shape=lambda: [])
    ...
    def test_load_model_passes_runtime_layout_kwargs_to_from_pretrained(self, monkeypatch) -> None:
        ...
        worker.load_config = SimpleNamespace(model_loader_extra_config={'dev_no': 2, 'target_cores': ['1:0'], 'target_clusters': [0, 1], 'core_mode': 'global8'})
        worker.model_config = SimpleNamespace(model='mobilint/test-model', hf_config=SimpleNamespace(model_type='qwen2'), model_kwargs={}, hf_overrides={})
        worker.vllm_config = SimpleNamespace(load_config=SimpleNamespace(model_loader_extra_config={}), model_config=worker.model_config)
        fake_model = SimpleNamespace(eval=lambda: None, get_input_embeddings=lambda: SimpleNamespace(), get_cache_mxq_model=lambda: SimpleNamespace())
        calls = []
